...
int main(int argc, char *argv[])
{
    ...
    // Vector auxiliar para fuerzas en paralelo y reutilizado en aceleraciones en paralelo
    ...
    for (int k = 0; k < num_iterations; k++)
    {
        ...
        for (int i = 0; i < num_objects; i++)
        {
            ...
            {   
                ...
                x_thread[threads_id][i] = ((objetos[i].fx) / (masa));
                vx_thread[threads_id][i] = ((objetos[i].vx) + ((x_thread[threads_id][i]) * (time_step)));
                px_thread[threads_id][i] = (objetos[i].x +((vx_thread[threads_id][i]) * (time_step)));
                y_thread[threads_id][i] = ((objetos[i].fy) / (masa));
                vy_thread[threads_id][i] = ((objetos[i].vy) + ((y_thread[threads_id][i]) * (time_step)));
                py_thread[threads_id][i] = (objetos[i].y + ((vy_thread[threads_id][i]) * (time_step)));
                z_thread[threads_id][i] = ((objetos[i].fz) / (masa));
                vz_thread[threads_id][i] = ((objetos[i].vz) + ((z_thread[threads_id][i]) * (time_step)));
                py_thread[threads_id][i] = (objetos[i].z + ((vz_thread[threads_id][i]) * (time_step)));
