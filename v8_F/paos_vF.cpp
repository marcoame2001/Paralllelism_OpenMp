...
int main(int argc, char *argv[])
{
    ...
    omp_set_num_threads(4); // Si quitamos esto se crean tantos hilos como nucleos el PC
    ...
    // Vector auxiliar para fuerzas en paralelo
    ...
    // Vector auxiliar para aceleraciones en paralelo
    ...
    // Vector auxiliar para velocidades en paralelo
    ...
    // Vector auxiliar para posiciones en paralelo
    ...
    for (int i = 0; i < nthreads; ++i) 
    {
        for (int j = 0; j < num_objects; ++j) 
        {
            if (objetos[j].existe)
            {             
                px_thread[i][j] = objetos[j].x;
                py_thread[i][j] = objetos[j].y;
                pz_thread[i][j] = objetos[j].z;
            }
        }
    }  
    */ 
    ...
    for (int k = 0; k < num_iterations; k++)
    {
                ...
                for (int j = 0; j < num_objects; j++)
                {
                    ...
                    {   
                        ...
                        {      
                            ...
                            //threads_id = omp_get_thread_num();  
                            //printf("Soy el hilo con ID: %i y calcule K=%i, I=%i, J=%i, Valor=%f\n",threads_id,k,i,j, x_thread[threads_id][i]);
        ...
        // Almaceno fuerzas en objeto y reseteo vector a 0 para reutilizarlo en aceleraciones
        for (int i = 0; i < nthreads; ++i) 
        {
            for (int j = 0; j < num_objects; ++j) 
            {
        ...
        for (int i = 0; i < num_objects; i++)
        {
            ...
            {   
                ...
                if (px_thread[threads_id][i]<=0)
                {
                    px_thread[threads_id][i] = 0;
                    vx_thread[threads_id][i] = ((-1)*(vx_thread[threads_id][i]));
                    printf("Rebote del objeto %i\n",i);
                }
                else if(px_thread[threads_id][i]>=size_enclosure)
                {
                    px_thread[threads_id][i] = (size_enclosure);
                    vx_thread[threads_id][i] = ((-1)*(vx_thread[threads_id][i]));
                    printf("Rebote del objeto %i\n",i);
                }
                if (py_thread[threads_id][i]<=0)
                {
                    py_thread[threads_id][i] = 0;
                    vy_thread[threads_id][i] = ((-1)*(vy_thread[threads_id][i]));
                    printf("Rebote del objeto %i\n",i);
                }
                else if(py_thread[threads_id][i]>=size_enclosure)
                {
                    py_thread[threads_id][i] = (size_enclosure);
                    vy_thread[threads_id][i] = ((-1)*(vy_thread[threads_id][i]));
                    printf("Rebote del objeto %i\n",i);
                }
                if (pz_thread[threads_id][i]<=0)
                {
                    pz_thread[threads_id][i] = 0;
                    vz_thread[threads_id][i] = ((-1)*(vz_thread[threads_id][i]));
                    printf("Rebote del objeto %i\n",i);
                }
                else if(pz_thread[threads_id][i]>=size_enclosure)
                {
                    pz_thread[threads_id][i] = (size_enclosure);
                    vz_thread[threads_id][i] = ((-1)*(vz_thread[threads_id][i]));
                    printf("Rebote del objeto %i\n",i);
                }
                ...
                std::cout << "Aceleracion X del objeto " << i << ": " << ax_thread[threads_id][i] << "\n";
                std::cout << "Aceleracion Y del objeto " << i << ": " << ay_thread[threads_id][i] << "\n";
                std::cout << "Aceleracion Z del objeto " << i << ": " << az_thread[threads_id][i] << "\n";
                std::cout << "Velocidad X del objeto " << i << ": " << vx_thread[threads_id][i] << "\n";
                std::cout << "Velocidad Y del objeto " << i << ": " << vy_thread[threads_id][i] << "\n";
                std::cout << "Velocidad Z del objeto " << i << ": " << vz_thread[threads_id][i] << "\n";
                std::cout << "Posicion X del objeto " << i << ": " << px_thread[threads_id][i] << "\n";
                std::cout << "Posicion Y del objeto " << i << ": " << py_thread[threads_id][i] << "\n";
                std::cout << "Posicion Z del objeto " << i << ": " << pz_thread[threads_id][i] << "\n";
                std::cout << "-------------------------------------------------------------\n";
            }
        }
