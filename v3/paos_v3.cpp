...
int main(int argc, char *argv[])
{
    ...
    omp_set_num_threads(1); // Si quitamos esto se crean tantos hilos como nucleos el PC
    int nthreads;
    #pragma omp parallel
    {
            ...
            if (objetos[i].existe)
            {
                ...
                for (int j = 0; j < num_objects; j++)
                {
                    ...
                    {   
                        ...
                        if (norm != 0) 
                        {   
                            fx = (gmm * ((objetos[j].x) - (objetos[i].x))) / (norm);
                            fy = (gmm * ((objetos[j].y) - (objetos[i].y))) / (norm);
                            fz = (gmm * ((objetos[j].z) - (objetos[i].z))) / (norm);

                            x_thread[threads_id][i] += fx;
                            y_thread[threads_id][i] += fy;
                            z_thread[threads_id][i] += fz;
                        }
                    }
